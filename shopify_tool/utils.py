"""General utility functions for the Shopify Fulfillment Tool.

This module provides helper functions that are used across different parts of
the application, such as determining platform-specific paths for data storage
and handling resource paths for bundled executables.
"""
...
def get_persistent_data_path(filename: str) -> str:
    ...
    It creates the application-specific directory if it does not already
    exist.
    ...
    Args:
        filename: The name of the file to be stored (e.g., "history.csv").

    Returns:
        The absolute path to the file within the application's data directory.
    """
...
def resource_path(relative_path: str) -> str:
    ...
    When an application is bundled with PyInstaller, its resources (like
    icons or data files) are stored in a temporary directory. The path to this
    ...
    development environment), it constructs the path relative to the
    project's root directory.

    Args:
        relative_path: The path to the resource relative to the application
            root (e.g., "data/templates/template.xls").

    Returns:
        The absolute path to the resource, suitable for the current execution
        environment.
    """
    try:
