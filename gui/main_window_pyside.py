...
class MainWindow(QMainWindow):
    ...
    def create_widgets(self):
        ...
        self.load_stock_btn = QPushButton("Load Stock File (.csv)"); self.stock_file_path_label = QLabel("Stock file not selected"); self.stock_file_status_label = QLabel(""); self.load_stock_btn.setEnabled(False)
        ...
        self.settings_button = QPushButton("⚙️"); self.settings_button.setFixedSize(40, 40)
    ...
    def log_activity(self, operation_type, description):
        ...
        self.activity_log_table.setItem(0, 1, QTableWidgetItem(operation_type))
        self.activity_log_table.setItem(0, 2, QTableWidgetItem(description))
    ...
    def create_new_session(self):
        try:
            ...
            self.load_orders_btn.setEnabled(True); self.load_stock_btn.setEnabled(True)
            self.log_activity("Session", f"New session started. Output: {self.session_path}")
        except Exception as e:
    ...
    def _post_analysis_ui_update(self):
        self.pandas_model = PandasModel(self.analysis_results_df)
        self.data_table.setModel(self.pandas_model)
        self.update_statistics_tab()
        self.packing_list_button.setEnabled(True); self.stock_export_button.setEnabled(True); self.report_builder_button.setEnabled(True)
        self.tab_view.setCurrentWidget(self.stats_tab)

    def run_analysis(self):
        if not self.session_path:
            QMessageBox.critical(self, "Session Error", "Please create a new session before running an analysis.")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Analysis Exception", f"An unexpected error occurred: {e}")
